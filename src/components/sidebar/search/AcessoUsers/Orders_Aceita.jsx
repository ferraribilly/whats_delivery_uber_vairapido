...
function Orders_Aceita({
  ...
  const handleCancelarMotorista = async () => {
    if (!formaPagamento) {
    ...
    const userFinal = {
      ...
      location: userRedux.location || userStorage.location || "minha localização",
    };

   

    const contruindoMonitoramentoViagem = {
      ...
      motorista: {
        ...
        distancia: contact.distancia,
        tempoChegada: contact.tempoChegada,
      },
    };

    try {
      await axios.post(`${ENDPOINT}/orders/resposta/monitoramento`, contruindoMonitoramentoViagem);

      localStorage.setItem("viagem_atual", JSON.stringify(contruindoMonitoramentoViagem));
  ...
  return (
    <li className="list-none  w-[100%] h-auto  px-[10px]">
      <div
        className="relative flex flex-col items-center py-10 gap-2  dark:bg-dark_bg_2 shadow-md mt-10"
      ...
      >
        ...
        <div className="flex flex-col items-center mt-12 gap-1 w-full">
          ...
          <h1 className="font-bold text-lg text-center dark:text-white">{contact.name}</h1>
          ...
          <h1 className="font-bold text-lg dark:text-black">{contact.location}</h1>
          ...
          <div className="w-full mt-4 px-4">
            ...
            <div className="flex justify-between bg-dark_bg_2 p-4 rounded-xl shadow-md">
              {[
          ...
          {tempoRestante !== null && !mostrarReacao && (
            <div className="w-full px-4 mt-2">
              ...
              <p className="font-bold text-lg dark:text-gold text-center">
          ...
          {mostrarReacao && (
            <div className="flex flex-col items-center mt-4 gap-2">
              <span className="text-red-600 font-bold text-center ">Não aceitou seu pedido sinto muito mas tente... outros motorista?</span>
          ...
          <div className="flex gap-2 mt-2">
            <button
              onClick={handleCancelarMotorista}
              className="bg-blue-500 hover:bg-green-600 hover:text-black text-white px-12 py-1 rounded text-sm shadow"
            >
              Solicitação
            </button>
...
// Socket da Location
const OrdersWithContext = (props) => (
  <SocketContext.Consumer>
    {(socket) => <Orders_Aceita {...props} socket={socket} />}
  </SocketContext.Consumer>
);

export default OrdersWithContext;
